...
class MenuTrailByPathActiveTrail extends MenuActiveTrail {
  ...
  public function getActiveTrailLink($menu_name) {
    ...
    if ($matches) {
      $match = $this->getBestMatch($matches, $menu_name);
      return $match;
    }
    return NULL;
  }
  ...
  /**
   * @param \Drupal\Core\Menu\MenuLinkInterface[] $matches
   * @param string $menuName
   * @return \Drupal\Core\Menu\MenuLinkInterface
   */
  private function getBestMatch(array $matches, string $menuName) {
    if (count($matches) === 1) {
      return reset($matches);
    }
    $requestUrl = \Drupal::request()->getRequestUri();

    /** @var \Drupal\Core\Menu\MenuLinkInterface $match */
    foreach ($matches as $match) {
      /** @var \Drupal\Core\Menu\MenuLinkInterface $parentMenuLink */
      $parentMenuLink = \Drupal::service('plugin.manager.menu.link')->getInstance(['id' => $match->getParent()]);
      if ($parentMenuLink && strpos($requestUrl, $parentMenuLink->getUrlObject()->toString()) === 0) {
        return $match;
      }
    }
    // No matching url found, just return the first one
    return reset($matches);
  }
}
