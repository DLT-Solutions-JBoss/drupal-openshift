...
use Drupal\page_manager_ui\Form\VisibilitySubform;
...
abstract class PanelsBlockConfigureFormBase extends FormBase {
  ...
  /**
   * The visibility condition plugin collection.
   *
   ...
   */
  public function __construct(SharedTempStoreFactory $tempstore, PluginManagerInterface $condition_manager, LanguageManagerInterface $language_manager, ContextRepositoryInterface $context_repository) {
  ...
  protected function getVisibilityConditions() {
    if (!isset($this->visibilityCollection)) {
      $configuration = $this->block->getConfiguration();
      $visibility = empty($configuration['visibility']) ? [] : $configuration['visibility'];
      $this->visibilityCollection = new ConditionPluginCollection($this->conditionManager, $visibility);
    }
